...
namespace RPGraph
{
    ...
    GraphLayout::GraphLayout(CSRUGraph &graph, float width, float height)
    ...
    void GraphLayout::writePositions(const char *path)
    {
        std::ofstream out_file(path);
        for (nid_t n = 0; n < graph.num_nodes(); ++n)
        {
            out_file << graph.offset_to_nid[n] << " " << getX(n) << " " << getY(n) << "\n";
        }
    }
}
