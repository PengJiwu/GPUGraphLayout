...
namespace RPGraph
{
    FA2Layout::FA2Layout(CSRUGraph &graph, float width, float height)
    ...
    void FA2Layout::apply_attract(nid_t n)
    {
        ...
        for (nid_t e = 0; e < graph.degree(n); ++e)
        {
            nid_t t = graph.nbr_id_for_node(n, e);
            ...
            else if (dissuade_hubs)
            {
                f_a_over_d = 1.0 / mass(n);
            }
